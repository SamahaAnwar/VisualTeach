...
class Model(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self.loss_fn(sk_feat, img_feat, neg_feat)
        self.log('test_loss', loss)  # Log the loss for monitoring

        # You can return any additional information you want to use or analyze
        return {'test_loss': loss, 'sk_feat': sk_feat, 'img_feat': img_feat, 'category': category}
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        # Aggregate and analyze the test outputs as needed
        avg_loss = torch.stack([x['test_loss'] for x in test_step_outputs]).mean()
        self.log('avg_test_loss', avg_loss)

        # If you need to perform additional analysis or logging, you can do it here
        # For example, concatenate features and categories for further analysis
        all_sk_feats = torch.cat([x['sk_feat'] for x in test_step_outputs])
        all_img_feats = torch.cat([x['img_feat'] for x in test_step_outputs])
        all_categories = np.array(sum([list(x['category']) for x in test_step_outputs], []))
    #=========================================================================== 
